...
class DynamicConvDecoder(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.layers = nn.ModuleList(
            [
                DynamicConvDecoderLayer(
                    config, kernel_size=config.decoder_kernel_size_list[i]
                )
                for i in range(config.decoder_layers)
            ]
        )
        self.layer_norm = (
            nn.LayerNorm(config.hidden_size) if config.final_norm else None
        )
        ...
        if config.adaptive_softmax_cutoff is not None:
            ...
            self.embed_out = nn.Parameter(
                torch.Tensor(config.vocab_size, output_embed_dim)
            )
    ...
    def forward(
        self, prev_target, contexts, incremental_state=None, use_layers=None, **kwargs
    ):
        ...
        return BaseModelOutput(
            last_hidden_state=X, hidden_states=inner_states, attentions=attn
        )
...
class DynamicConvDecoderLayer(nn.Module):
    def __init__(self, config, kernel_size):
        ...
        if config.decoder_glu:
            ...
            self.linear1 = nn.Linear(self.embed_dim, self.conv_dim)
            self.act = None
        ...
        self.context_fc = nn.Linear(context_size, self.embed_dim)
    ...
    def forward(self, X, contexts, incremental_state):
        ...
        for context_type in ["image", "article", "faces", "obj"]:
            ...
            X_context = self.maybe_layer_norm(
                self.context_attn_lns[context_type], X, before=True
            )
            ...
            X_context = self.maybe_layer_norm(
                self.context_attn_lns[context_type], X_context, after=True
            )
