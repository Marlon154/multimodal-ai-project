...
class DynamicConvDecoderConfig(PretrainedConfig):
    def __init__(self, vocab_size, max_position_embeddings, hidden_size, num_attention_heads,
                 ...
                 max_target_positions, dropout, share_decoder_input_output_embed,
                 decoder_output_dim, decoder_conv_dim, decoder_glu,
                 decoder_conv_type, weight_softmax, weight_dropout, relu_dropout, input_dropout,
                 decoder_normalize_before, decoder_kernel_size_list, adaptive_softmax_cutoff,
                 tie_adaptive_weights, adaptive_softmax_dropout, tie_adaptive_proj,
                 adaptive_softmax_factor, decoder_layers, final_norm, padding_idx,
                 namespace, section_attn, article_embed_size, **kwargs):
...
class DynamicConvDecoderLayer(nn.Module):
    def __init__(self, config, kernel_size):
        ...
        self.context_attns['image'] = MultiHeadAttention(
            self.embed_dim, config.num_attention_heads, kdim=2048, vdim=2048,
            dropout=config.attention_probs_dropout_prob)
        self.context_attn_lns['image'] = nn.LayerNorm(self.embed_dim)

        self.context_attns['article'] = MultiHeadAttention(
            # self.embed_dim, config.num_attention_heads, kdim=config.article_embed_size, vdim=config.article_embed_size,
            self.embed_dim, config.num_attention_heads, kdim=1024, vdim=1024,
            dropout=config.attention_probs_dropout_prob)
        self.context_attn_lns['article'] = nn.LayerNorm(self.embed_dim)

        self.context_attns['faces'] = MultiHeadAttention(
            self.embed_dim, config.num_attention_heads, kdim=512, vdim=512,
            dropout=config.attention_probs_dropout_prob)
    ...
    def forward(self, X, contexts, incremental_state):
        ...
        for context_type in ['image', 'article', 'faces', 'obj']:
            ...
            X_context, attn = self.context_attns[context_type](query=X_context,
